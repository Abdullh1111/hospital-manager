...
export const useSignupViewModel = (navigation: any) => {
  ...
  const setFirstName = React.useCallback(
    (value: string) => dispatch(setAuthForm({firstName: value})),
    [dispatch],
  );
  const setLastName = React.useCallback(
    (value: string) => dispatch(setAuthForm({lastName: value})),
    [dispatch],
  );
  const setEmail = React.useCallback(
    (value: string) => dispatch(setAuthForm({email: value})),
    [dispatch],
  );
  const setPassword = React.useCallback(
    (value: string) => dispatch(setAuthForm({password: value})),
    [dispatch],
  );
  const setSex = React.useCallback(
    (value: string) => dispatch(setAuthForm({sex: value})),
    [dispatch],
  );
  ...
  const setCountry = useCallback(
    (value: string) => dispatch(setAuthForm({country: value})),
    [dispatch],
  );
  const setCity = useCallback(
    (value: string) => dispatch(setAuthForm({city: value})),
    [dispatch],
  );
  const setDob = useCallback(
    (value: string) => dispatch(setAuthForm({dob: value})),
    [dispatch],
  );
  /**
  ...
  const handleSignup = useCallback(async () => {
    ...
    if (!country) {
      Alert.alert(
        t('missingData', 'Missing Data'),
        t('errors.selectCountry', 'Please select your country'),
      );
      return;
    }
    if (!city) {
      Alert.alert(
        t('missingData', 'Missing Data'),
        t('errors.selectCity', 'Please select your city'),
      );
      return;
    }
    if (!dob) {
      Alert.alert(
        t('missingData', 'Missing Data'),
        t('errors.selectDob', 'Please select your date of birth'),
      );
      return;
    }
    ...
    try {
      const sendOTPResponse = await axiosInstance.post(URLS.sendOTP, {
        email: email.trim(),
      });
      dispatch(otpSuccess());
      Alert.alert(t('success', 'Success'), sendOTPResponse.data.message);
      ...
      const errorMessage =
        ...
        t(
          'errors.failedToSendResetLink',
          'Failed to send reset link. Please try again.',
        );
      dispatch(signupFailure(errorMessage));

      Alert.alert(t('error', 'An error occurred'), errorMessage);
    }
  }, [
    firstName,
    lastName,
  ...
  const handleSignupSubmitAfterOTP = useCallback(
    async (Otp: string) => {
      // Dispatch signup request action
      dispatch(signupRequest());
      ...
      try {
        ...
        const userToStore = {
          ...
          expireTimeStamp: new Date(
            Date.now() + response.data.expiresIn * 60 * 1000,
          ).toISOString(),
        };
        ...
        const errorMessage =
          err?.response?.data?.message ||
          t('errors.failedToRegister', 'Registration failed');
        dispatch(signupFailure(errorMessage));

        Alert.alert(t('error', 'An error occurred'), errorMessage);
      }
    },
    [email, firstName, lastName, sex, role, password, dispatch, t, navigation, dob, city, country],
  );
